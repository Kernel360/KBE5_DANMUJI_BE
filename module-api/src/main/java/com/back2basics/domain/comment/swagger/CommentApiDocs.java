    ...
    @ApiResponses(value = {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            ...
            content = @Content(schema = @Schema(implementation = ApiResponse.class),
                examples = @ExampleObject(value = CommentDocsResult.COMMENT_READ_LIST_SUCCESS))),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "404", description = "게시글을 찾을 수 없음",
            content = @Content(examples = @ExampleObject(value = CommentDocsResult.POST_NOT_FOUND)))
    })
    ResponseEntity<ApiResponse<List<CommentReadResponse>>> getCommentsByPostId(
        @Parameter(hidden = true) @AuthenticationPrincipal CustomUserDetails customUserDetails,
