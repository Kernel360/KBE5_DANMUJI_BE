...
public record TodoListResponse(Long id, String content, Long postId, LocalDateTime createdAt,
                               boolean isChecked, LocalDateTime checkedAt) {

    public static TodoListResponse from(TodoListResult result) {
        return new TodoListResponse(result.id(), result.content(), result.postId(),
