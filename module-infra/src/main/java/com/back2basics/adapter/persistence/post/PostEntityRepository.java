...
public interface PostEntityRepository extends JpaRepository<PostEntity, Long> {
    ...
    @Query("SELECT p FROM PostEntity p WHERE p.authorId = :authorId AND p.deletedAt IS NULL")
    List<PostEntity> findActivePostsByAuthorId(@Param("authorId") Long authorId);

    @Query("SELECT p FROM PostEntity p WHERE p.deletedAt IS NULL ORDER BY p.priority DESC, p.createdAt DESC")
    List<PostEntity> findActivePostsOrderByPriority();
